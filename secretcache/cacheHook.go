...
// CacheHook is an interface to hook into the local in-memory secretcache. This interface will allow
// users to perform actions on the items being stored in the in-memory
// secretcache. One example would be encrypting/decrypting items stored in the
// in-memory secretcache.
type CacheHook interface {
	// Put prepares the object for storing in the secretcache.
