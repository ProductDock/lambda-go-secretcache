...
func TestPutIfAbsent(t *testing.T) {
	...
	if !addedToCache {
		t.Fatalf("Failed initial add to secretcache")
	}

	addedToCache = lruCache.putIfAbsent(key, data*2)

	if addedToCache {
		t.Fatalf("Should have failed second add to secretcache")
	}

	retrievedItem, found := lruCache.cacheMap[key]

	if !found {
		t.Fatalf("Did not find expected entry in secretcache")
	}
	...
	if lruCache.cacheSize != 1 {
		t.Fatalf("Expected secretcache size to be 1")
	}
}
...
func TestGet(t *testing.T) {
	...
	if found {
		t.Fatalf("Did not expect entry in secretcache")
	}
...
func TestLRUCacheMax(t *testing.T) {
	...
	for i := 0; i <= 90; i++ {
		if _, found := lruCache.get(strconv.Itoa(i)); found {
			t.Fatalf("Found unexpected val in secretcache - %d", i)
		}
	}

	for i := 91; i <= 100; i++ {
		if val, found := lruCache.get(strconv.Itoa(i)); !found || i != val.(int) {
			t.Fatalf("Expected to find val in secretcache - %d", i)
		}
	}
}
...
func TestLRUCacheEmpty(t *testing.T) {
	...
	if found {
		t.Fatalf("Found unexpected val in secretcache")
	}
}
...
func TestLRUCacheRecent(t *testing.T) {
	...
	if val, found := lruCache.get("0"); !found || 0 != val.(int) {
		t.Fatalf("Expected to find val in secretcache - %d", 0)
	}
}
