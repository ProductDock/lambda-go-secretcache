...
// SecretCacheItem maintains a secretcache of secret versions.
...
// newSecretCacheItem initialises a secretCacheItem using default secretcache size and sets next refresh time to now
...
func (ci *secretCacheItem) executeRefresh(ctx context.Context) (*secretsmanager.DescribeSecretOutput, error) {
	...
	if maxTTL < 0 {
		return nil, &InvalidConfigError{
			baseError{
				Message: "cannot set negative ttl on secretcache",
			},
		}
	} else if maxTTL < 2 {
		ttl = maxTTL
...
// getVersion gets the secret secretcache version associated with the given stage.
...
// setWithHook sets the secretcache item's data using the CacheHook, if one is configured.
...
// getWithHook gets the secretcache item's data using the CacheHook, if one is configured.
