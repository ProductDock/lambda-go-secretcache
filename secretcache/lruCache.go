...
// lruCache is a secretcache implementation using a map and doubly linked list.
...
// lruItem is the secretcache item to hold data and linked list pointers.
type lruItem struct {
	next *lruItem
	prev *lruItem
	key  string
...
// putIfAbsent puts an lruItem initialised from the given data in the secretcache.
// Updates head of the linked list to be the new lruItem.
// If secretcache size is over max allowed size, removes the tail item from secretcache.
// Returns true if new key is inserted to secretcache, false if it already existed.
